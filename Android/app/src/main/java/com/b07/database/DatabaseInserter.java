...
public class DatabaseInserter {
  ...
  /**
   * Use this to insert a new user without hash its password (because it's already hashed).
   * 
   * @param name the user's name.
   * @param age the user's age.
   * @param address the user's address.
   * @param password the user's password (hashsed).
   * @param connection the database connection.
   * @return the user id
   * @throws DatabaseInsertException if there is a failure on the insert
   */
  protected static int insertNewUserHashedPwd(String name, int age, String address, String password,
      Connection connection) throws DatabaseInsertException {
    int id = insertUser(name, age, address, connection);
    if (id != -1) {
      insertHashedPwd(id, password, connection);
      return id;
    }
    throw new DatabaseInsertException();
  }
  ...
  // For deserialization use only, insert a already hashed password into database
  private static boolean insertHashedPwd(int userId, String hashedPwd, Connection connection) {
    String sql = "INSERT INTO USERPW(USERID, PASSWORD) VALUES(?,?);";
    try {
      PreparedStatement preparedStatement = connection.prepareStatement(sql);
      preparedStatement.setInt(1, userId);
